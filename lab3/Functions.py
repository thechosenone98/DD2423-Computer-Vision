...
#
def showgrey(Image, display=True, res=64, zmin=-1, zmax=-1, plot=plt):
    if zmin == -1:
        zmin = np.min(Image)
    if zmax == -1:
        zmax = np.max(Image)
    ...
    plot.imshow(np.array(im, dtype=np.uint8), cmap, aspect="auto")
    plot.axis('image')
    plot.axis('off')
    if display:
        plot.show()
...
def showfs(Image, display=True, res=64, plot=plt):
    showgrey(np.log(1 + np.abs(np.fft.fftshift(Image))),
             display, res, plot=plot)
...
def variance(inpic):
    ...
    [x, y] = np.meshgrid(np.linspace(-(usize-1)/2, (usize-1)/2, int(usize)),
                         np.linspace(-(vsize-1)/2, (vsize-1)/2, int(vsize)))
    abspic = np.abs(inpic)
    norm = np.sum(abspic)
...
def overlaycurves(img, curves, plot=False):
    [h, w] = np.shape(img)
...
def overlay_bounds(I, segm):
    ...
    grow[0:h, 0:w] = segm
    mask = (segm != grow[1:h+1, 0:w]) | (segm !=
                                         grow[0:h, 1:w+1]) | (segm != grow[1:h+1, 1:w+1])
